...
class DEM(nn.Module):
    ...
    def predict(self, image, word_embeddings):
        with torch.no_grad():
            ...
            visual_emb = visual_emb.repeat(
                1, n_class).view(batch_size * n_class, -1)
            semantic_emb = self.word_emb_transformer(
                word_embeddings).repeat(batch_size, 1)

            score = (visual_emb -
                     semantic_emb).norm(dim=1).view(batch_size, n_class)
            pred = score.min(1)[1]
...
class DeViSE(nn.Module):
    ...
    def forward(self, image, word_embeddings):
        ...
        scores = torch.mul(visual_emb, semantic_emb).sum(
    ...
    def get_loss(self, image, label, word_embeddings, LAMBDA=0.5):
        ...
        loss = F.cross_entropy(logtis, label) * LAMBDA + \
            (1 - LAMBDA) * F.multi_margin_loss(scores, label)
        return loss
